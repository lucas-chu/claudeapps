...
Creates the shared sandbox environment and the ClaudeFather agent. Both are
persistent, versioned objects: never create them in the per-message path.
Teammate agents are NOT created here — ClaudeFather creates those at hire time.
...
from app.prompts import CLAUDEFATHER_SYSTEM, CREATE_TEAMMATE_TOOL, LIST_TEAMMATES_TOOL
...
def main() -> int:
    ...
    if config.CLAUDEFATHER_AGENT_ID:
        print(f"ClaudeFather already exists ({config.CLAUDEFATHER_AGENT_ID}); updating it.")
        version = managed_agent.update_agent_system(
            config.CLAUDEFATHER_AGENT_ID, CLAUDEFATHER_SYSTEM
        )
        agent_id = config.CLAUDEFATHER_AGENT_ID
    else:
        agent_id, version = managed_agent.create_agent(
            name="ClaudeFather",
            system=CLAUDEFATHER_SYSTEM,
            tools=[
        ...
        print(f"Created ClaudeFather agent {agent_id} (version {version})")

    print("\nPaste into .env:\n")
    print(f"ENVIRONMENT_ID={environment_id}")
    print(f"CLAUDEFATHER_AGENT_ID={agent_id}")
    print(f"CLAUDEFATHER_AGENT_VERSION={version if version is not None else ''}")
